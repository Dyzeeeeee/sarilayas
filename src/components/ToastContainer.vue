<template>
  <div class="fixed top-4 right-4 md:top-auto md:bottom-4 md:right-4 z-[80] space-y-2 pointer-events-none">
    <TransitionGroup name="toast" tag="div">
      <div
        v-for="toast in toasts"
        :key="toast.id"
        class="pointer-events-auto min-w-[300px] max-w-md rounded-lg shadow-lg border overflow-hidden animate-slide-in flex"
        :class="toastClasses[toast.type]"
      >
        <!-- Image Preview -->
        <div v-if="toast.image" class="w-20 h-20 shrink-0 bg-gray-100 overflow-hidden">
          <img
            :src="toast.image"
            :alt="toast.message"
        ...
        <!-- Content -->
        <div class="flex-1 p-4 flex items-start space-x-3">
          <div class="shrink-0">
